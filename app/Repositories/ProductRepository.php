...
class ProductRepository extends BaseRepository
{
    ...
    public function allProductsWithMainImages()
    {
        $status = array_keys(Constant::STATUS);
        $products = $this->model::selectRaw("`products`.*, '' as `detail`")->with([
            'images' => function ($query) use ($status) {
    ...
    public function filterProducts($filters)
    {
        ...
        if (!empty($filters['category'])) {
            $query->where('category_id', $filters['category'])
                ->orWhereHas("category", function ($subquery) use ($filters) {
                    $subquery->where("parent_id", $filters['category']);
                });
        }
        ...
        if (!empty($filters['color'])) {
            $query->whereHas('images', function ($subquery) use ($filters) {
        ...
        if (!empty($filters['tag'])) {
            info($filters);
            $query->whereHas('productTags', function ($subquery) use ($filters) {
                $subquery->where('tag_id', $filters['tag']);
            });
        }

        // Get product default has table related
        $status = array_keys(Constant::STATUS);
        $query->selectRaw("`products`.*, '' as `detail`")->with([
            'images' => function ($query) use ($status) {
                $query->where("status", $status[0])->orderBy('level');
            },
            'productTags.tag',
